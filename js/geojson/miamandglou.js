;(function (window) {
    // geoJson definitions
    window.geoInfoFood = [
        {
            'type': 'Feature',
            'properties': {
                'name': 'Relais de l\'Eglise',
                'link': 'http://minetest.amelieonline.net/doku.php?id=village:allee_centrale#restaurant_le_relais_de_l_eglise',
                'icon': 'fa-utensils',
                'image': 't=1612714095&w=500&h=264&tok=c78e22&media=screenshot_20210207_170323.png',
                'description': 'Restaurant chaleureux et familial, on y mange tout plein de spécialités !'
            },
            'geometry': {
                'type': 'Point',
                'coordinates': [8877,10543]
            }
        },
        {
            'type': 'Feature',
            'properties': {
                'name': 'Pub le Medieval',
                'link': 'http://minetest.amelieonline.net/doku.php?id=village:chateau_royal#pub_le_medieval',
                'icon': 'fa-beer',
                'image': 't=1612714159&w=500&h=264&tok=c68dfa&media=screenshot_20210207_170450.png',
                'description': 'Lieu à la mode très underground, on y boit d\'excellents crus de bière écossaise sous une ambiance gothique.'
            },
            'geometry': {
                'type': 'Point',
                'coordinates': [9146,10413]
            }
        },
        {
            'type': 'Feature',
            'properties': {
                'name': 'Cafétéria du Forum',
                'link': 'http://minetest.amelieonline.net/doku.php?id=village:forum#cafeteria',
                'icon': 'fa-utensils',
                'image': 't=1612714397&w=500&h=264&tok=864e93&media=screenshot_20210207_170607.png',
                'description': 'Fast-food, on y mange donc des hamburgers faits maison, des hot-dogs, des sucreries et on y boit des sodas !'
            },
            'geometry': {
                'type': 'Point',
                'coordinates': [8932,10682]
            }
        },
        {
            'type': 'Feature',
            'properties': {
                'name': 'Sushi shop',
                'link': 'http://minetest.amelieonline.net/doku.php?id=village:allee_centrale#sushi_shop',
                'icon': 'fa-utensils',
                'image': 't=1612714978&w=500&h=264&tok=efa94d&media=screenshot_20210207_171824.png',
                'description': 'Venez vous sustanter de sushis et sashimis !'
            },
            'geometry': {
                'type': 'Point',
                'coordinates': [8877,10580]
            }
        },
    ]
}(window))
